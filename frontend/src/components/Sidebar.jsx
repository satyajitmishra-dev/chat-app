...
const Sidebar = () => {
  ...
  <div className="border-b border-base-300 w-full p-4 flex flex-col gap-3">
    ...
    {/* Make toggle visible even on mobile */}
    <div className="flex items-center justify-center lg:justify-start gap-2 text-xs lg:text-sm">
      <label className="cursor-pointer flex items-center gap-2">
        ...
        />
        <span className="hidden sm:inline">Show online</span>
      </label>
      <span className="text-zinc-500">({onlineUsers?.length - 1})</span>
    </div>
  </div>
  ...
  <div className="overflow-y-auto flex-1 py-3">
    {filteredUsers?.map((user) => (
      <button
        ...
        className={`w-full flex items-center gap-3 p-2 hover:bg-base-300 transition ${
    ...
    {filteredUsers.length === 0 && (
      <div className="text-center text-orange-300 py-4">No online users</div>
    )}
  </div>
</aside>
